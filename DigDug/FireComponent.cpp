...
void dae::FireComponent::Update()
{
	...
	if (m_Timer <= 0.f) {
		...
		if(m_Direction == "Left"){
			GetGameObject()->GetTransform()->Translate(m_OriginLocation.x - GetGameObject()->GetComponent<TextureComponent>()->GetRect().w, m_OriginLocation.y);
		}
	}
...
void dae::FireComponent::Init()
{
	...
	if (comp) {
		...
		GetGameObject()->GetTransform()->Translate(m_OriginLocation);
		...
		if (direction == MathLib::Movement::LEFT) {
			...
			//GetGameObject()->GetTransform()->SetScale(glm::vec2{ 2.4f, 2.4f });
			m_Direction = "Right";
			GetGameObject()->GetTransform()->Translate(static_cast<float>(m_EnemyGameObject->GetComponent<TextureComponent>()->GetRect().w), 0);
		}
